...
def mip_optimize(params, site_types, bonds, layers, partitions, numb):
    ...
    layer_contribution = model.addVar(vtype=GRB.CONTINUOUS, lb=-float("inf"), ub=float("inf"))
    ...
    if layer_energy == 0:
        ...
        decision = dict([(i, model.addVar(vtype=GRB.BINARY)) for i in layer_indices])
        deltap = dict([(i, model.addVar(vtype=GRB.CONTINUOUS, lb=0)) for i in layer_indices])
        deltam = dict([(i, model.addVar(vtype=GRB.CONTINUOUS, lb=0)) for i in layer_indices])

        for i, layer in enumerate(layers):
            model.addConstr( deltap[i] - deltam[i] == 2 * sum([x[j] for j in layer]) - len(layer) )
    ...
    model.addConstr( sum([x[e] for e in partitions[0]]) >= sum([x[e] for e in partitions[1]]))
    model.addConstr( sum([x[e] for e in partitions[2]]) >= sum([x[e] for e in partitions[3]]))
    model.addConstr( sum([x[e] for e in partitions[4]]) >= sum([x[e] for e in partitions[5]]))
...
def run(size, element):
    ...
    pos = octahedron.build_nanoparticle(size)
    site_types, bonds, layers, partitions = octahedron.get_attributes(pos)
    ...
    objective, xs = mip_optimize(params, site_types, bonds, layers, partitions, numb)
...
def main():
    ...
    if len(sys.argv) != 3:
        raise Exception("arguments: [size] [element]")
